  ...
  >
    <td class="tw-w-1/3">
      <div class="tw-flex tw-items-center">
        ...
        >
          <p class="tw-text-xs tw-font-bold tw-leading-3 tw-text-white">{{ avatar }}</p>
        </div>
        <div class="tw-pl-2">
          <p class="tw-text-sm tw-font-medium tw-leading-none dark:tw-text-white">{{ user.name }}</p>
          <p
            class="tw-text-xs tw-leading-3 tw-text-gray-400 tw-mt-2"
          >{{ user.permissions.join(', ') }}</p>
        </div>
      </div>
    </td>
    <td class="tw-w-1/4">
      <div class="tw-flex tw-items-center tw-text-gray-400">{{ user.account }}</div>
    </td>
    <td class="tw-w-1/4">
      <div
        class="tw-flex tw-items-center tw-text-gray-400"
      >{{ formatTimestamp(user.createTime * 1000) }}</div>
    </td>
    <td>
      <q-btn color="primary" class="tw-mr-4" text-color="black" @click="showEdit = true">编辑</q-btn>
      <q-btn
        v-if="user.id !== 1"
        flat
