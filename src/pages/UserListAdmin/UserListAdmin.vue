    ...
    <div class="q-dark tw-w-full tw-rounded tw-overflow-hidden">
      <div class="tw-px-4 tw-py-4">
        <div class="tw-flex tw-items-center tw-justify-between">
          ...
          >用户管理</p>
          <div class="tw-mt-0">
            <q-btn flat class="tw-text-white" icon="add" @click="showCreate = true"></q-btn>
          </div>
        </div>
      </div>
      <div class="q-dark tw-px-4 tw-pb-5">
        <div class="tw-overflow-x-auto">
          <table class="tw-w-full tw-whitespace-nowrap">
            <tbody>
              <UserItem v-for="user in userList" :key="user.id" :user="user" @modify="refreshList"></UserItem>
            </tbody>
          </table>
        </div>
      </div>
      <div class="q-pa-lg flex flex-center">
        <q-pagination v-model="page" color="black" :max="totalPage" :boundary-numbers="false" />
      </div>
