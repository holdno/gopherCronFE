<template>
  <div class="tw-w-full tw-h-full tw-flex tw-flex-col">
    <div class="tw-flex tw-flex-row tw-gap-4 tw-flex-wrap">
      <q-btn flat type="primary" :disable="!canUpdate" @click="updateWorkFlow">保存</q-btn>
      <q-btn flat icon="refresh" title="刷新" @click="() => refresh()">刷新</q-btn>
      <q-btn flat icon="restart_alt" title="重置视图" @click="() => workflow.ResetView()">重置视图</q-btn>
      <q-btn flat icon="add" title="添加新任务节点" @click="() => workflow.ShowAddNodeDialog()">添加新任务节点</q-btn>
      <q-btn
        flat
        icon="delete"
    ...
    <WorkFlow ref="workflow" v-model="current" :workflow-id="props.id" :tasks="tasks" />
  </div>
</template>
