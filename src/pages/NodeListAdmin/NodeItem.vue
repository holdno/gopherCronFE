    ...
    <td class="tw-w-2/3">
      <div class="tw-flex tw-items-center">
        ...
        <div class="tw-pl-2">
          ...
          >{{ node.clientIP }}</p>
          <p class="tw-text-xs tw-leading-3 tw-text-gray-400 tw-mt-2">{{ node.version }}</p>
        </div>
      </div>
    </td>
    <td>
      <div class="tw-w-min tw-flex tw-scale-75 md:tw-scale-100">
        <q-btn flat class="tw-text-red-300" @click="showReload = true">重载配置</q-btn>
      </div>
    </td>
  </tr>
  <q-dialog v-model="showReload">
    <q-card>
      ...
      <q-card-actions align="right">
        ...
        <q-btn flat label="确定" color="primary" :loading="loading" @click="reload" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
