<template>
  <q-dialog v-model="show">
    <Confirm
      v-model="showConfirm"
      type="warning"
      :content="`确定要移除Webhook ${selected?.types} 吗？`"
      @confirm="
        ...
        <ul role="list" class="tw-divide-y tw-divide-white/5">
          ...
          >
            <div class="tw-min-w-0 tw-flex-auto">
              ...
              >
                <p class="tw-truncate">{{ getHookType(data.types) }}</p>
                <svg
                  viewbox="0 0 2 2"
                ...
                <p v-if="data.createTime" class="tw-whitespace-nowrap">
                  {{
                    formatTimestamp(data.createTime * 1000, 'YYYY-MM-DD HH:mm')
                  }}
                </p>
              </div>
            </div>
...
<script setup lang="ts">
  ...
  async function removeWebhook(data: WebHook) {
    loading.value = true;
    try {
      await deleteWebhook(data.projectId as number, data.types);
      await fetchWebhooks();
    } finally {
      loading.value = false;
    }
