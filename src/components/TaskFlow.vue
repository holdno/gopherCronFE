<template>
  ...
  <q-menu ref="viewMenu" touch-position context-menu>
    <q-list dense>
      <q-item clickable v-close-popup key="add-task">
      ...
      <template v-if="canAddEdge">
        <q-item
          ...
          key="add-edge"
        >
          <q-item-section>添加指向：{{ edgeName(selectedNodes.reverse()) }}</q-item-section>
        </q-item>
      </template>
      <template v-if="selectedEdges.length > 0">
        <q-item clickable v-close-popup @click="() => removeEdges(selectedEdges)" key="remove-edge">
          <q-item-section>删除指向：{{ selectedEdges.join(", ") }}</q-item-section>
        </q-item>
      </template>
      <template v-if="selectedNodes.length > 0">
        <q-item
          ...
          key="remove-task"
        >
          <q-item-section>删除节点：{{ selectedNodes.join(", ") }}</q-item-section>
        </q-item>
      </template>
      <q-separator />
      <q-item clickable v-close-popup @click="resetView" key="reset-view">
        <q-item-section>重置视图</q-item-section>
      </q-item>
      <q-item clickable v-close-popup @click="reset" key="reset">
...
import { Nodes, Edges, Layouts, ViewEvent, EventHandlers, UserConfigs, NodeLabelDirection } from 'v-network-graph'
  ...
  methods: {
    ...
    removeEdges(edges: string[]) {
      for (let edge of edges) {
        let [source, target] = edgeParts(edge)
    ...
    updateEdges() {
      for (let edge of Object.keys(this.edges)) {
      ...
      for (let task of this.current) {
        ...
        for (let parentId of task.deps) {
          this.edges[edgeName([parentId, task.id])] = { "source": parentId, "target": task.id }
        }
      }
    },
    updateLayouts() {
      let nodes = this.layouts.nodes;
  ...
  computed: {
    eventHandlers(): EventHandlers {
      ...
      return {
        ...
        "node:select": (nodes) => { hideContextMenu() },
        "edge:select": (edges) => { hideContextMenu() },
      }
    },
    canAddEdge(): boolean {
